...
- **Actiwatch** → Actiware 6, using the manufacturer's rest-interval detection.
...
volunteers (who were part of an unrelated study) contributed between 2 and 14 
nights each (median 11). Because the volunteers were recruited over that period
rather than measured in a single block, the dates are spread out; device model,
acquisition settings and scoring software version were unchanged throughout.
